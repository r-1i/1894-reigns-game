...
public class CardSpawner : MonoBehaviour, IEventReceiver<OnDestroyCardEvent> {
  ...
  // Закомментированы поля шансов, так как логика шансов отключена из за пакостей дипсика в
  // генерации json'ов карточек
  /*
  ...
  */
  ...
  public void LoadCards() {
    ...
    TextAsset[] textAssets = Resources.LoadAll<TextAsset>("Cards/nikolay2");
  ...
  private void SpawnDeck() {
    ...
    */
    ...
    while (deck.Count < 3)
    {
        var candidates = loadedCards.Where(card => card.is_start &&
    !alreadyAdded.Contains(card.id)).ToList(); if (candidates.Count == 0) break;
